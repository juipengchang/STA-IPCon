...
class ConvTransformerBackbone(nn.Module):
        ...
        for idx in range(arch[0]):
            ...
            self.embd.append(
                MaskedConv1D(
                    ...
                    SpAtten=SpAtten if idx == 0 else False,
                    TeAtten=TeAtten if idx == 0 else False,
                    PreTrain=PreTrain,
                    RandomPatch=RandomPatch if idx == 0 else False,
                )
            )
            if with_ln:
    ...
    def forward(self, x, mask):
        ...
        for idx in range(len(self.embd)):
            if idx == 0:
                x, mask, foreground, background = self.embd[idx](x, mask, True)
            else:
        ...
        return out_feats, out_masks, foreground, background
...
class ConvBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.ModuleList()
        for idx in range(arch[1]):
            self.stem.append(ConvBlock(n_embd, 3, 1, PreTrain=pretrain))

        # main branch using convs with pooling
        self.branch = nn.ModuleList()
        for idx in range(arch[2]):
            self.branch.append(ConvBlock(n_embd, 3, self.scale_factor, PreTrain=pretrain))
