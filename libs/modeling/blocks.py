...
class MaskedConv1D(nn.Module):
    ...
    def __init__(
        ...
        groups=1,
        bias=True,
        padding_mode='zeros', SpAtten=False, TeAtten=False, PreTrain=False, RandomPatch=False,
    ):
        super().__init__()
        ...
        if self.SpAtten:
            self.attention_op = nn.Sequential(
                ...
                nn.Sigmoid(),
                # nn.AvgPool2d(kernel_size=(8, 1) if in_channels == 4096 else (1, 1), stride=(8, 1) if in_channels == 4096 else (1, 1), padding=(0, 0))
            )
        self.TeAtten = TeAtten
        self.lstm_op = ConvLSTM(input_dim=1,
                                hidden_dim=[8, 8, 1],
                                kernel_size=(3, 3),
                                num_layers=3,
                                batch_first=True,
        ...
        self.random_patch_relu = nn.ReLU()
        self.pretrain = PreTrain
        if self.pretrain:
            self.conv_1 = nn.Sequential(
                nn.Linear(4096, 1024),
                nn.Linear(1024, 256)
                # nn.Conv2d(1, 8, (1, 1), (1, 1), (0, 0)),
            )
    ...
    def forward(self, x, mask, first=False):
        ...
        if self.SpAtten and self.TeAtten:
            x_resize = torch.unsqueeze(x, dim=1).permute(0, 3, 1, 2).reshape(B, T, 1, -1, 64)
            x_lstm, _ = self.lstm_op(x_resize)
            x_lstm = (x_lstm[0] + x_resize).reshape(B * T, 1, -1, 64)
            attention_map = torch.squeeze(self.attention_op(x_lstm)).reshape(B, T, C).permute(0, 2, 1)
            if self.pretrain:
                foreground = torch.squeeze(x_lstm).reshape(B, T, C).permute(0, 2, 1) * attention_map
                background = torch.squeeze(x_lstm).reshape(B, T, C).permute(0, 2, 1) * (1 - attention_map)
                foreground = self.conv_1(foreground.permute(0, 2, 1).reshape(B * T, -1)).reshape(B, T, -1).permute(0, 2, 1)
                background = self.conv_1(background.permute(0, 2, 1).reshape(B * T, -1)).reshape(B, T, -1).permute(0, 2, 1)
            x = x + x * attention_map

        if self.SpAtten and not self.TeAtten:
            x_resize = torch.unsqueeze(x, dim=1).permute(0, 3, 1, 2).reshape(B * T, 1, 64, 64)
            attention_map = torch.squeeze(self.attention_op(x_resize)).reshape(B, T, C).permute(0, 2, 1)
            x = x + x * attention_map

        if not self.SpAtten and self.TeAtten:
            x_resize = torch.unsqueeze(x, dim=1).permute(0, 3, 1, 2).reshape(B, T, 1, 64, 64)
            x_lstm, _ = self.lstm_op(x_resize)
            attention_map = self.sigmoid(x_lstm[0].reshape(B * T, 1, 64, 64)).reshape(B, T, 1, 64, 64)
            x = (attention_map * x_resize + x_resize).reshape(B, T, C).permute(0, 2, 1)

        # import cv2
        # import os
        # if not os.path.exists('test_pic'):
        #     os.mkdir('test_pic')
        # a = attention_map.permute(0, 2, 1).reshape(B, T, 1, 64, 64) * 255
        # cv2.imwrite('test_pic/attention_map.png', a[0, 4, 0].cpu().numpy())
        # cv2.imwrite('test_pic/attention_map_minus.png', 255 - a[0, 4, 0].cpu().numpy())
        # cv2.imwrite('test_pic/ori_feature.png', (x_lstm.reshape(B, T, 1, 64, 64)[0, 4, 0] * 255).cpu().numpy())
        # cv2.imwrite('test_pic/foreground.png', (x_lstm.reshape(B, T, 1, 64, 64)[0, 4, 0] * a[0, 4, 0]).cpu().numpy())
        # cv2.imwrite('test_pic/background.png', (x_lstm.reshape(B, T, 1, 64, 64)[0, 4, 0] * (255 - a[0, 4, 0])).cpu().numpy())
        # for i in [0, 4, 5, 6, 7]:
        #     cv2.imwrite('test_pic/t' + str(i) + '.png', 255 * x[0, :, i].reshape(64, 64).cpu().numpy())

        if self.random_patch:
            x = self.conv_random_patch(x)

        # for i in [0, 4, 5, 6, 7]:
        #     b = x[0, :, i].reshape(64, 64)
        #     b = b / torch.max(b)
        #     print(torch.max(b), torch.min(b))
        #     cv2.imwrite('test_pic/t' + str(i) + '_1.png', 255 * b.cpu().numpy())
        # exit()

        # if self.SpAtten or self.TeAtten:
        #     import cv2
        #     import os
        #     if not os.path.exists('test_pic'):
        #         os.mkdir('test_pic')
        #     if not self.SpAtten and self.TeAtten:
        #         save = attention_map * 255
        #     else:
        #         save = attention_map.permute(0, 2, 1).reshape(B, T, 1, 64, 64) * 255
        #     # for i in range(T):
        #     for i in range(4, 11):
        #         cv2.imwrite('test_pic/test_' + str(i) + '.png', save[0, i, 0].cpu().numpy())
        #     exit()
        ...
        if not first:
            ...
            return out_conv, out_mask, foreground, background

    def conv_random_patch(self, video_list):
        conv = torch.nn.Conv2d(1, 1, (5, 5), stride=(1, 1), padding=(2, 2)).cuda()
        for i in range(len(video_list)):
            feats = video_list[i].reshape(64, 64, -1)
            index = torch.randint(2, 61, [2])
            x = int(index[0])
            y = int(index[1])
            conv.weight.data = torch.unsqueeze(torch.unsqueeze(feats[x - 2: x + 3, y - 2: y + 3, 0], dim=0), dim=0)
            a = torch.unsqueeze(feats, dim=0).permute(3, 0, 1, 2)
            b = conv(a)
            c = b.permute(1, 2, 3, 0)
            d = torch.squeeze(c).reshape(64 * 64, -1)
            video_list[i] = self.random_patch_relu(d)
        return video_list
...
class MaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        self.query_conv = MaskedConv1D(
            ...
            stride=stride, padding=padding, groups=self.n_embd, bias=False, PreTrain=PreTrain
        )
...
class ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = MaskedConv1D(
            n_embd, width, kernel_size, n_ds_stride, padding=padding, PreTrain=PreTrain)
        self.conv2 = MaskedConv1D(
            width, n_out, kernel_size, 1, padding=padding, PreTrain=PreTrain)

        # attach downsampling conv op
        if n_ds_stride > 1:
            # 1x1 strided conv (same as resnet)
            self.downsample = MaskedConv1D(n_embd, n_out, 1, n_ds_stride, PreTrain=PreTrain)
        else:
