...
def get_p_value(arrA, arrB, tho):
    c = [abs(i - j) for i, j in zip(arrA, arrB)]
    c = np.array(c)
    t, p = stats.ttest_1samp(c, tho, alternative='less')
    # t, p = sw.ztest(c, value=tho, alternative='smaller')
    return t, p
...
def make_scheduler(
    ...
    if optimizer_config["warmup"]:
        ...
        if model['pretrain'] or model['random_patch']:
            max_epochs += 10
...
def train_one_epoch(
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        losses = model(video_list, curr_epoch)
...
def valid_one_epoch(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            output, ad, p, r, f, micro_p, micro_r, micro_f, t1, t2, t3, t4, t5, t6 = model(video_list, decodeSetting=decodeSetting)
            # print(video_list[0]['video_id'], t1, t2, t3, t4, t5, t6)
            ad_list.append(ad)
            p_list.append(p)
            r_list.append(r)
            f_list.append(f)
    ...
    if printInfo:
        print('Average Deviation is {:.2f}'.format(sum(ad_list) / len(ad_list)))
        print('Macro Precision is {:.4f}'.format(sum(p_list) / len(p_list)))
        print('Macro Recall is {:.4f}'.format(sum(r_list) / len(r_list)))
        print('Macro F1-score is {:.4f}'.format(sum(f_list) / len(f_list)))
        print('Micro Precision is {:.4f}'.format(sum(micro_p_list) / len(micro_p_list)))
        print('Micro Recall is {:.4f}'.format(sum(micro_r_list) / len(micro_r_list)))
        print('Micro F1-score is {:.4f}'.format(sum(micro_f_list) / len(micro_f_list)))
        for tho in [5.2 + 0.2 * i for i in range(6)]:
            t, p = get_p_value(t1_list + t2_list + t3_list, t4_list + t5_list + t6_list, tho)
            print('tho is {:.2f}, t value is {:.2f}, p value is {:.2f} '.format(tho, t, p))
        t_list = [abs(i - j) for i, j in zip(t1_list + t2_list + t3_list, t4_list + t5_list + t6_list)]
        mean = np.mean(t_list)
        se = stats.sem(t_list)
        # for k in range(len(t_list)):
        #     print('a[' + str(k + 1) + '] <- ' + str(t_list[k]))
        print('mean:{:.2f}'.format(mean) + ', se:{:.2f}'.format(se) + ', confidence interval:{:.2f}'.format(mean - 1.653 * se) + '-{:.2f}'.format(mean + 1.653 * se))
    ...
    return mAP, sum(ad_list) / len(ad_list), sum(f_list) / len(f_list)
